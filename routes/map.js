...
var logger = winston.loggers.get('map');
var map = {};
...
router.get('/', function (req, res) {
    ...
    if (req.query.from != null && req.query.to != null) {
        ...
        var to = req.query.to;
        logger.info('Received request to navigate', { from: from, to: to });
        var route = findPath(from, to);
        res.status(200).send({
            data: route
        });
    }
    // Looking for <call> and optional <collection> query parameter
    else if (req.query.call != null) {
        var call = req.query.call;
        var collection = (req.query.collection != null ? req.query.collection : null);
        logger.info('Received request to navigate', { call: call, collection: collection });
        ...
        res.status(400).send('Missing query parameters, <from> and <to> OR <call> and optional <collection>');
    }
});
